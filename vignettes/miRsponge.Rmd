---
title: "miRsponge: identification and analysis of miRNA sponge interaction networks and modules"
author: "\\
...
To accelerate the research of miRNA sponge, we develop an R package ‘miRsponge’ to implement popular methods in the identification and analysis of miRNA sponge network and module.
  


# Identification of miRNA sponge interactions
We implement seven popular methods (miRHomology [13, 14], PC [15, 16], SPPC [17], PPC [11], Hermes [18], MuTaME [19], and CERNIA [20]) to identify miRNA sponge interactions. The seven methods should meet a basic condition: the significance of sharing of miRNAs by each RNA-RNA pair (e.g. adjusted p-value < 0.01). Each method has its own merit due to different evaluating indicators. Thus, we present an integrate method to combine predicted miRNA sponge interactions from different methods. 

## miRHomology function

In miRsponge package, we implement 'miRHomology' function based on the homology of miRNA binding sites present in the 3'UTR sequence.
```{r}
...
'PC' function consider expression data based on 'miRHomology' function. Significantly positive miRNA sponge interaction pairs are reserved. 
...
In miRsponge package, we implement 'SPPC' function based on sensitivity correlation (difference between the Pearson and partial correlation coefficients).
...
In miRsponge package, 'Hermes' function predicts competing endogenous RNAs via evidence for competition for miRNA regulation based on conditional mutual information.

```{r}
miR2Target <- system.file("extdata", "miR2Target.csv", package="miRsponge")
ExpData <- system.file("extdata", "ExpData.csv", package="miRsponge")
HermesceRInt <- Hermes(miR2Target, ExpData, num_perm =5)
head(HermesceRInt)
```
...
In miRsponge package, 'PPC' function is a variant of 'Hermes' function. However, it predicts competing endogenous RNAs via evidence for competition for miRNA regulation based on Partial Pearson Correlation.

```{r}
miR2Target <- system.file("extdata", "miR2Target.csv", package="miRsponge")
ExpData <- system.file("extdata", "ExpData.csv", package="miRsponge")
PPCceRInt <- PPC(miR2Target, ExpData, num_perm =5)
head(PPCceRInt)
```
...
To obtain a list high-confidence miRNA sponge interactions, we implement 'Integrate' function to integrate results of different methods. 
...
IntegrateceRInt <- Integrate(Interlist, Intersect_num=3)
head(IntegrateceRInt)
```

Parameter 'Intersect_num' is used to set the least number of methods intersected for integration. That is to say, we only reserve those miRNA sponge interactions predicted at least 'Intersect_num' methods. 

# Validation of miRNA sponge interactions

To validate the predicted miRNA sponge interactions, we implement 'SpongeValidate' function. The groundtruth of miRNA sponge interactions are from miRSponge v2.0 (<http://www.bio-bigdata.net/miRSponge/>).
...
# Module identification of miRNA sponge interaction network

To further understand the module-level properties of miRNA sponges in cancer, we implement 'NetModule' function to identify miRNA sponge modules. Possible methods include FN [21], MCL [22], LINKCOMM [23] and MCODE [24].
...
We implement 'ModuleDEA' function to make disease enrichment analysis of modules. Possible disease databases include DO: Disease Ontology database (<http://disease-ontology.org/>), DGN: DisGeNET database (<http://www.disgenet.org/>) and NCG: Network of Cancer Genes database (<http://ncg.kcl.ac.uk/>). Moreover, 'ModuleFEA' function is implemented to conduct functional GO, KEGG and Reactome enrichment analysis of modules. Possible ontology databases contain GO: Gene Ontology database (<http://www.geneontology.org/>), KEGG: Kyoto Encyclopedia of Genes and Genomes Pathway Database (<http://www.genome.jp/kegg/>), and Reactome: Reactome Pathway Database (<http://reactome.org/>).
...
sponge_Module_Survival <- ModuleSurvival(spongenetwork_Cluster, ExpData, SurvData)
sponge_Module_Survival
```

# Conclusions
miRsponge package provides methods to study miRNA sponge (also called ceRNA or miRNA decoy), including popular methods for identifying miRNA sponge interactions, integrate method to integrate miRNA sponge interactions from different methods, function to validate miRNA sponge interactions, function to infer miRNA sponge modules, functions to conduct enrichment analysis of modules, function to make survival analysis of modules. It could provide a useful tool for the research of miRNA sponge.
...
[11] Le TD, Zhang J, Liu L, et al. Computational methods for identifying miRNA sponge interactions. Brief Bioinform. 2016, DOI: 10.1093/bib/bbw042.
...
[20] Sardina DS, Alaimo S, Ferro A, et al. A novel computational method for inferring competing endogenous interactions. Brief Bioinform. 2016, DOI: 10.1093/bib/bbw084.
