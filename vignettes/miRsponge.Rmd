...
We implement seven popular methods (miRHomology [13, 14], PC [15, 16], 
SPPC [17], PPC [11], Hermes [18], MuTaME [19], and CERNIA [20]) to 
identify miRNA sponge interactions. The seven methods should meet a 
basic condition: the significance of sharing of miRNAs by each RNA-RNA 
pair (e.g. adjusted p-value < 0.01). Each method has its own merit due 
to different evaluating indicators. Thus, we present an integrate method 
to combine predicted miRNA sponge interactions from different methods. 

## miRHomology function

In miRsponge package, we implement 'miRHomology' function based on the 
homology of miRNA binding sites present in the 3'UTR sequence.
```{r}
...
'PC' function consider expression data based on 'miRHomology' function. 
Significantly positive miRNA sponge interaction pairs are reserved. 
...
In miRsponge package, we implement 'SPPC' function based on sensitivity 
...
In miRsponge package, 'Hermes' function predicts competing endogenous RNAs 
...
In miRsponge package, 'PPC' function is a variant of 'Hermes' function. 
...
In miRsponge package, we implement 'MuTaME' function based on the logarithm 
of four scores: (1) the fraction of coomon miRNAs, (2) the density of the 
...
To evaluate the strength of each RNA-RNA pair, we further normalize combined 
...
In miRsponge package, we implement 'CERNIA' function based on the 
logarithm of seven scores: (1) the fraction of coomon miRNAs, (2) 
...
combined scores to obtain normalized scores with interval [0 1].
...
To obtain a list high-confidence miRNA sponge interactions, we 
implement 'Integrate' function to integrate results of different methods. 
...
miRNA sponge interactions predicted at least 'Intersect_num' methods. 
...
are from miRSponge v2.0 (<http://www.bio-bigdata.net/miRSponge/>).
...
Possible methods include FN [21], MCL [22], LINKCOMM [23] and MCODE [24].
...
modules. Possible disease databases include DO: Disease 
...
enrichment analysis of modules. Possible ontology databases 
...
# Conclusions
miRsponge package provides methods to study miRNA sponge (also called 
ceRNA or miRNA decoy), including popular methods for identifying miRNA 
sponge interactions, integrate method to integrate miRNA sponge interactions 
from different methods, function to validate miRNA sponge interactions, 
function to infer miRNA sponge modules, functions to conduct enrichment 
analysis of modules, function to make survival analysis of modules. 
It could provide a useful tool for the research of miRNA sponge.
...
of gene and pseudogene mRNAs regulates tumour biology. Nature, 2
010, 465:1033–8.
...
miRNA sponge interactions. Brief Bioinform. 2016, DOI: 10.1093/bib/bbw042.
