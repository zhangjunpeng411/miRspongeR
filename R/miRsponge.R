    ...
    while ( npar > 0 )
    {
     ...
     Nex  <- length( apor )
     margave <- floor( ( marg[ npar, 1 : cdim ] + 
     marg[ npar, ( cdim + 1 ) : dim2 ] ) / 2 )
	 ...
	 # decide partition or not
         ...
         Nxx <- apply(marg[npar,(cdim+1):dim2]
         - marg[npar,1:cdim] + matrix(1, nrow = 1,ncol = cdim), 2, prod);
    ...
    ## PredCMI: Predict competing endogenous RNAs via evidence for competition for miRNA 
    ## regulation based on conditional mutual information
    ...
    ## miRNA regulation based on partial pearson correlation. A variant of PredCMI function.
    ...
    for ( j in 1 : num_cand )
    {
       ...
       tri_id[ idx_tri, ] <- c( exprRownames[ idx_tar ],
			       exprRownames[ idx_miR ],
			       exprRownames[ idx_mod ] )
    ...
    R <- do.call( "fun", lapply( clusterApply( cl=cl, x=splitRows, 
    get("%*%"), B), enquote ) )
    } else {
    R <- A %*% B
    }
    ...
    ## The first step of DT-Hybrid recommendation algorithm: generating the weights for each pair of target nodes
    ...
    ## The second step of DT-Hybrid recommendation algorithm: generating ecommendation scores of each RNA-RNA pair
    ...
    ## For input miR2Target, the miRNA-target interactions could be miRNA-mRNA, miRNA-lncRNA, 
    ## miRNA-circRNA, miRNA-pseudogene, etc.
    ## For input MREs, each row contains five elements: Mirna, Target, energy, gap_l, gap_r.

    ## 1. miRHomology
    miRHomology <- function( miR2Target, minSharedmiR=3, padjustvaluecutoff=0.01, 
    padjustmethod="BH" )
    {
    ...
    # Extract RNA-RNA pair based on the homology of miRNA binding sites present in the 3'UTR sequence
    ...
    # Extract positive correlated RNA-RNA pairs, the p-values are adjusted by BH method in default.
    ceRInt <- ceRInt[which((p.adjust(C[, 2], method=padjustmethod) < padjustvaluecutoff & 
    C[, 3]>poscorcutoff & p.adjust(C[, 4], method=padjustmethod) < padjustvaluecutoff) == 'TRUE'), ]
    C <- C[ which((p.adjust(C[, 2], method=padjustmethod) < padjustvaluecutoff & 
    C[, 3]>poscorcutoff & p.adjust(C[, 4], method=padjustmethod) < padjustvaluecutoff) == 'TRUE'), ]
    ...
    ceRInt <- ceRInt[which((p.adjust(C[, 2], method=padjustmethod) < padjustvaluecutoff & 
    C[, 3] > poscorcutoff & p.adjust(C[, 4], method=padjustmethod) < padjustvaluecutoff & 
    ...
    # Extract significant RNA-RNA pairs, the p-values are adjusted by BH method in default.
    ceRInt <- ceRInt[which((p.adjust(C[, 2], method=padjustmethod) < padjustvaluecutoff & 
    p.adjust(C[, 3], method=padjustmethod) < padjustvaluecutoff) == 'TRUE'), ]
    C <- C[which((p.adjust(C[, 2], method=padjustmethod) < padjustvaluecutoff & 
    p.adjust(C[, 3], method=padjustmethod) < padjustvaluecutoff) == 'TRUE'), ]
    ...
    "p.adjusted_value of shared miRNAs", "p.adjusted_value of RNA competition")
    } else {
    ...
    ## 6. MuTaME method 
    MuTaME <- function (miR2Target, MREs, minSharedmiR=3, padjustvaluecutoff=0.01, 
    ...
    C <- matrix(NA, m2*(m2-1)/2, 8)
        ...
        for (i in 1:(m2-1)){
		 ...
		 C[(i-1)*m2+j-sum(1:i), 5] <- sum(sapply(cm, function (miR) {
                 positions <- SharedMREs[SharedMREs[, 1] == miR, c("gap_l", "gap_r")] # get positions of the MREs
                 if (nrow(positions) <= 0) return (1)
                 return (log(abs(max(positions[, 1]) - min(positions[, 2]))^2/sum((positions[, 2]-positions[, 1])^2)))
                 }))
		 ...
		 C[(i-1)*m2+j-sum(1:i), 6] <- log( ( B-length(unique(SharedMREs[, 1])) -1 ) / B )
                 ...
                 C[(i-1)*m2+j-sum(1:i), 7] <- C[(i-1)*m2+j-sum(1:i), 3]+C[(i-1)*m2+j-sum(1:i), 4]
		 +C[(i-1)*m2+j-sum(1:i), 5]+C[(i-1)*m2+j-sum(1:i), 6]
    ...
    ## 7. CERNIA method 
    CERNIA <- function (miR2Target, MREs, ExpDatacsv, minSharedmiR=3, poscorcutoff=0, 
    ...
    for (i in 1:(m2-1)){
		 ...
		 if (M3>=minSharedmiR & M5<padjustvaluecutoff & M6>poscorcutoff & nrow(SharedMREs)>0){
		 ...
		 cerna.recommendations <- DTHybrid(miRTarget)
                 C[(i-1)*m2+j-sum(1:i), 8] <- cerna.recommendations[targetSym[i], targetSym[j]]

		 # Score 7 for the pairwise Peason correlation between putative RNA-RNA pair expression data
		 C[(i-1)*m2+j-sum(1:i), 9] <- log(M6)

                 C[(i-1)*m2+j-sum(1:i), 10] <- C[(i-1)*m2+j-sum(1:i), 3]+C[(i-1)*m2+j-sum(1:i), 4] +
		 C[(i-1)*m2+j-sum(1:i), 5]+C[(i-1)*m2+j-sum(1:i), 6]+C[(i-1)*m2+j-sum(1:i), 7] +
		 C[(i-1)*m2+j-sum(1:i), 8]+C[(i-1)*m2+j-sum(1:i), 9]
    ...
    # Extract RNA-RNA pair based on seven scores, the p-values are adjusted by BH method in default.
    ceRInt <- ceRInt[apply(ceRInt, 1, function(x) !all(is.na(x))), ]
    C <- C[apply(C, 1, function(x) !all(is.na(x))), ]
    C[, 11] <- (C[, 10]-min(C[, 10]))/(max(C[, 10])-min(C[, 10]))
    ceRInt <- ceRInt[which((p.adjust(C[, 2], method=padjustmethod) < padjustvaluecutoff 
    & C[, 11] > scorecutoff) == 'TRUE'), ]
    C <- C[which((p.adjust(C[, 2], method=padjustmethod) < padjustvaluecutoff 
    & C[, 11] > scorecutoff) == 'TRUE'), ]
    ...
    if(length(Interlist)>=2 & length(Interlist)>=Intersect_num){
		  ...
		  for (i in 1:Combnum){
		       Interin <- do.call(rbind,
		       lapply(Combcase[i, ], function(i) Interlist[[i]]))				       
    ...
    entrezIDs <- lapply(1:length(Modulelist), function(i) as.character(entrezIDs[[i]]))
    enrichDOs <- lapply(1:length(Modulelist), function(i) enrichDO(entrezIDs[[i]], ont=ont, 
    pvalueCutoff=padjustvaluecutoff, pAdjustMethod=padjustedmethod))
    enrichDGNs <- lapply(1:length(Modulelist), function(i) enrichDGN(entrezIDs[[i]], 
    pvalueCutoff=padjustvaluecutoff, pAdjustMethod=padjustedmethod))
    enrichNCGs <- lapply(1:length(Modulelist), function(i) enrichNCG(entrezIDs[[i]], 
    pvalueCutoff=padjustvaluecutoff, pAdjustMethod=padjustedmethod))
    ...
    if ('try-error' %in% class(try_xx1)) stop("Enriched modules are not enough!\n")
    ...
    if ('try-error' %in% class(try_xx2)) stop("Enriched modules are not enough!\n")
    try_xx3 <- try(compareCluster(entrezIDs, fun="enrichNCG", pvalueCutoff=padjustvaluecutoff, 
    pAdjustMethod=padjustedmethod), silent=TRUE)
    if ('try-error' %in% class(try_xx3)) stop("Enriched modules are not enough!\n")
    ...
    {
    entrezIDs <- lapply(1:length(Modulelist), function(i) bitr(Modulelist[[i]], fromType="SYMBOL", 
    toType="ENTREZID", OrgDb=OrgDb)$ENTREZID)
    entrezIDs <- lapply(1:length(Modulelist), function(i) as.character(entrezIDs[[i]]))
    enrichGOs <- lapply(1:length(Modulelist), function(i) enrichGO(entrezIDs[[i]], OrgDb=OrgDb, ont = ont, 
    pvalueCutoff=padjustvaluecutoff, pAdjustMethod=padjustedmethod))
    enrichKEGGs <- lapply(1:length(Modulelist), function(i) enrichKEGG(entrezIDs[[i]], organism = KEGGorganism, 
    pvalueCutoff=padjustvaluecutoff, pAdjustMethod=padjustedmethod))
    enrichReactomes <- lapply(1:length(Modulelist), function(i) enrichPathway(entrezIDs[[i]], 
    organism = Reactomeorganism, pvalueCutoff=padjustvaluecutoff, pAdjustMethod=padjustedmethod))

    if(plot & length(Modulelist)>=2){
    ModuleID <- paste("M", 1:length(Modulelist), sep = "")
    names(entrezIDs) <- ModuleID
    try_xx1 <- try(compareCluster(entrezIDs, fun="enrichGO", OrgDb=OrgDb, ont=ont, 
    pvalueCutoff=padjustvaluecutoff, pAdjustMethod=padjustedmethod), silent=TRUE)
    if ('try-error' %in% class(try_xx1)) stop("Enriched modules are not enough!\n")
    try_xx2 <- try(compareCluster(entrezIDs,fun="enrichKEGG", organism=KEGGorganism, 
    pvalueCutoff=padjustvaluecutoff, pAdjustMethod=padjustedmethod), silent=TRUE)
    if ('try-error' %in% class(try_xx2)) stop("Enriched modules are not enough!\n")
    try_xx3 <- try(compareCluster(entrezIDs, fun="enrichPathway",organism=Reactomeorganism, 
    pvalueCutoff=padjustvaluecutoff, pAdjustMethod=padjustedmethod), silent=TRUE)
    if ('try-error' %in% class(try_xx3)) stop("Enriched modules are not enough!\n")
    xx1 <- compareCluster(entrezIDs, fun="enrichGO", OrgDb=OrgDb, ont=ont, 
    pvalueCutoff=padjustvaluecutoff, pAdjustMethod=padjustedmethod)
    xx2 <- compareCluster(entrezIDs, fun="enrichKEGG", organism=KEGGorganism, 
    pvalueCutoff=padjustvaluecutoff, pAdjustMethod=padjustedmethod)
    xx3 <- compareCluster(entrezIDs, fun="enrichPathway", organism=Reactomeorganism, 
     pvalueCutoff=padjustvaluecutoff, pAdjustMethod=padjustedmethod)
    dev.new()
    ...
    for (i in 1:length(Modulelist)){
       Interin_Data <- cbind(SurvData[, 2:3], ExpData[, which(ExpDataNames %in% Modulelist[[i]])])
       Interin_Data <- na.omit(Interin_Data)
       
       try_mm <- try(coxph(survival::Surv(time, status)~., data = data.frame(Interin_Data)), silent=TRUE)
    ...
    dev.new()
    plot(myfit[[i]], lty=1, col = c("red","green"), main=paste("Module",i), 
    ...
    rownames(LogRank_res) <- unlist( lapply( 1:length(myfit), function(i) paste("Module", i) ) )
    }
